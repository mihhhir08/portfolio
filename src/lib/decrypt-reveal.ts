// Cipher-glyph decrypt reveal.
//
// Adapted from a component built on the HTML-in-Canvas API (drawElementImage /
// layoutsubtree). That API's origin trial ran Chrome M148-M151 and has expired
// without shipping to stable, so it renders for nobody today. Only the *source
// of the content texture* depended on it — the glyph atlas, shape matching,
// decrypt circle, edge flicker and aberration are all shader-side. So the
// texture is now painted from the DOM directly and the shaders are unchanged
// apart from the final composite.
//
// Overlay model: this canvas paints an opaque cipher over its content and goes
// transparent inside the cursor circle, so the real DOM underneath shows
// through. The content stays interactive, selectable and crawlable — nothing is
// replaced, only veiled.
...
export interface DecryptRevealOptions {
  ...
  aspect?: number;
  /** Characters the cipher is written in. Shapes are matched automatically. */
  charset?: string;
  /** How much glyphs keep the color of the UI beneath them (0 to 1). */
  colored?: number;
  /** Cipher color as any CSS color. */
  color?: string;
  /** Brightness of the cipher glyphs (0.2 to 3). */
  brightness?: number;
  /** Minimum contrast the cipher keeps against the background (0 to 1). */
  legibility?: number;
  /** Contrast of the glyph shape matching (0.3 to 3). */
  contrast?: number;
  ...
  /** Width of the decrypting flicker band as a fraction of the radius. */
  edgeWidth?: number;
  ...
  edgeTint?: number;
  /** How much of the underlying UI bleeds through the cipher (0 to 1). */
  passthrough?: number;
  /** Contrast above which a cell counts as UI and earns a glyph. */
  threshold?: number;
  /** Backdrop color behind the content, as any CSS color. */
  background?: string;
  /** Seconds the decrypt circle takes to catch up with the cursor. */
  smoothing?: number;
}

export interface DecryptRevealElements {
  /** Offscreen canvas the DOM replica is painted into. */
  source: HTMLCanvasElement;
  /** The element being veiled; its text is sampled to build the texture. */
  content: HTMLElement;
...
export interface DecryptRevealInstance {
  ...
  /** Repaint the replica. Call when the content's text or layout changes. */
  refresh: () => void;
  destroy: () => void;
}
...
void main () {
  ...
  // Overlay composite. Alpha is keyed to whether this cell actually holds
  // content: cells with a glyph paint opaque (hiding the real text beneath),
  // empty cells stay fully transparent so the card itself shows through
  // untouched — no background matching, so hover tints and theme gradients
  // behind the veil can never betray it as a rectangle. Inside the decrypt
  // circle alpha falls to zero and the genuine DOM reads through.
  outColor = vec4(encrypted, (1.0 - e) * step(0.5, glyph));
}`;
...
/**
 * Paints a replica of the element's rendered text into the source canvas.
 *
 * This is what drawElementImage() used to do. Characters are placed from their
 * own Range rects, so wrapping and kerning come from the real layout rather
 * than being re-measured — the replica only has to be accurate enough to drive
...
function paintReplica(
  ...
  for (let node = walker.nextNode(); node; node = walker.nextNode()) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (!r.width && !r.height) continue;
      // baseline sits near the bottom of the glyph box; 0.79 tracks the
      // cap-height ratio closely enough for a texture this is sampled from
      ctx.fillText(ch, r.left - box.left, r.top - box.top + r.height * 0.79);
    }
  }
...
export function createDecryptReveal(
  ...
  function frame(now: number) {
    ...
    if (settled && !contentDirty && (reducedMotion || !churning)) {
  ...
  // Repaint once webfonts land, otherwise the replica is measured with
  // fallback metrics and the glyphs trace the wrong shapes.
